...
def batch_wer(references, hypotheses, pad_idx=0, start_idx=1, end_idx=2):
    ...
    #batch WER: total errors / total words
...
def make_5ch_from_image_gpu(img_path, blur_sigma=1.0, thick_radius=1, device="cuda"):
    ...
    return five.squeeze(0)    #batch dimension: (1, 5, H, W) -> (5, H, W)
...
def main():
    ...
    CHECKPOINT_DIR = 'checkpoints'
    LOG_DIR = 'logs'
    RESUME_FROM = '/kaggle/input/best-after-45-epochs/checkpoint_best (3).pth'
    ...
    best_checkpoint = os.path.join('/kaggle/input/best-after-45-epochs', 'checkpoint_epoch_45.pth')
